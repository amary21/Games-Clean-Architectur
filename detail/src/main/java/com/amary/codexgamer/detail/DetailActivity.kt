...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val query = intent?.getQueryParameter("gamesId")?.toInt()
        if (query != null) {
            initDetailGames(query, binding)
        }
    }

    @Suppress("DEPRECATION")
    @SuppressLint("SetJavaScriptEnabled")
    private fun initDetailGames(query: Int, binding: ActivityDetailBinding) {
        binding.apply {
            detailViewModel.getDetailGames(query).observe(this@DetailActivity, {gamesData ->
